...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!isValid) {
      console.error('❌ Invalid Bridge Signature');
    ...
    // Upsert ensures we don't crash if Bridge retries the same event ID
    await prisma.bridgeEvent.upsert({
      ...
      create: {
        id: event.id,
        type: event.type,
        payload: event, // Prisma handles JSON type automatically
      },
    });
