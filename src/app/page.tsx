...
export default function Home() {
  ...
  // 2. SET INITIAL URL
  useEffect(() => {
    if (kybUrl) setIframeUrl(generateWidgetUrl(kybUrl));
  }, [kybUrl, generateWidgetUrl]);

  // 3. LISTENERS FOR TAB HANDSHAKE
  useEffect(() => {
    // A. Listen for postMessage (Legacy/Direct)
    const handleMessage = (event: MessageEvent) => {
      if (event.origin !== window.location.origin) return;
      if (event.data?.type === 'BRIDGE_INQUIRY_RESUME' && event.data?.inquiryId) {
        console.log("Handshake (Message): Resuming", event.data.inquiryId);
        setIframeUrl(generateWidgetUrl(null, event.data.inquiryId));
      }
    };

    // B. Listen for LocalStorage (Broadcast/Cross-Tab)
    const handleStorage = (event: StorageEvent) => {
      if (event.key === 'bridge-handshake-signal' && event.newValue) {
        try {
          const data = JSON.parse(event.newValue);
          // Only react if the signal is fresh (within last 5 seconds) to avoid loops
          if (Date.now() - data.timestamp < 5000) {
            console.log("Handshake (Storage): Resuming", data.inquiryId);
            setIframeUrl(generateWidgetUrl(null, data.inquiryId));

            // Clear signal so it doesn't fire again on refresh
  ...
  // ... (Keep Load Session, Webhooks, Customer Status logic exactly as before)
  useEffect(() => {
  ...
  // ... (Keep Handlers exactly as before)
