      ...
      if (raw) {
        try {
          ...
          if (Date.now() - data.timestamp < 30000) {
            console.log("[Main] Poller found fresh signal:", data);

            // HANDLE "RESUME" (We have an ID)
            if (data.type === 'BRIDGE_INQUIRY_RESUME' && data.inquiryId) {
              const resumeUrl = generateWidgetUrl(null, data.inquiryId);
              setIframeUrl(resumeUrl);
            }
            // HANDLE "REFRESH ONLY" (No ID, just update status)
            else if (data.type === 'BRIDGE_REFRESH_ONLY') {
              console.log("[Main] Refreshing customer status only.");
              fetchCustomerStatus(); // Just update the dashboard
              setIframeUrl(null); // Hide iframe as flow might be done
            }
